...
def get_dataset(dataset, data_path):
    if dataset == 'MNIST':
        ...
        transform = transforms.Compose([transforms.Resize(im_size), transforms.Grayscale(num_output_channels=3), transforms.ToTensor(), transforms.Normalize(mean=mean, std=std)])
        dst_train = datasets.MNIST(root="./digit_data", train=True, download=True, transform=transform) # no augmentation
        dst_test = datasets.MNIST(root="./digit_data", train=False, download=True, transform=transform)
        ...
        dst_train = datasets.SVHN(root="./digit_data", split='train', download=True, transform=transform)  # no augmentation
        dst_test = datasets.SVHN(root="./digit_data", split='test', download=True, transform=transform)
        class_names = [str(c) for c in range(num_classes)]
        ...
        im_size = (28, 28)
        num_classes = 10
        mean = [0.2473, 0.2473, 0.2473]
        std = [0.2665, 0.2665, 0.2665]
        ...
        dst_train = datasets.USPS(root="./digit_data", train=True, transform=transform, download=True)  # no augmentation
        dst_test = datasets.USPS(root="./digit_data", train=False, transform=transform, download=True)
        class_names = [str(c) for c in range(num_classes)]
        ...
        im_size = (28, 28)
        num_classes = 10
        mean = [0.4828, 0.4603, 0.4320]
        std = [0.1960, 0.1938, 0.1977]
        transform = transforms.Compose([transforms.Resize(im_size), transforms.ToTensor(), transforms.Normalize(mean=mean, std=std)])
        dst_train = datasets.ImageFolder('./digit_data/synthetic_digits/imgs_train', transform=transform)
        dst_test = datasets.ImageFolder('./digit_data/synthetic_digits/imgs_valid', transform=transform)
        class_names = [str(c) for c in range(num_classes)]
        ...
        im_size = (28, 28)
        num_classes = 10
        mean = [0.4595, 0.4629, 0.4097]
        std = [0.1727, 0.1603, 0.1785]
        transform = transforms.Compose([transforms.Resize(im_size), transforms.ToTensor(), transforms.Normalize(mean=mean, std=std)])
        dst_train = datasets.ImageFolder('./digit_data/mnistm/train', transform=transform)
        dst_test = datasets.ImageFolder('./digit_data/mnistm/test', transform=transform)
        class_names = [str(c) for c in range(num_classes)]
