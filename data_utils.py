...
class Cifar100CDataset(Dataset):
    def __init__(self, data_path, channels, partition, filename=None, train=True, transform=None):
        if filename is None:
            if train:
                data = np.load(os.path.join(data_path, f'train/train_part{partition}.npz'))
                self.images, self.labels = data['images'], data['labels']
            else:
                data = np.load(os.path.join(data_path, f'val/val_part{partition}.npz'))
                self.images, self.labels = data['images'], data['labels']
        else:
